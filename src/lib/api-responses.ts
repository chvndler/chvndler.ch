...
export function success(res: NextApiResponse, json: {[key: string]: unknown} = {}) {
  return res.status(200).json(json)
}

export function badRequest(res: NextApiResponse, error: unknown = 'Bad Request') {
  console.error(error)
  return res.status(400).json({error: formatError(error)})
}

export function notAuthorized(res: NextApiResponse, error: unknown = 'Not Authorized') {
  console.error(error)
...
export function internalServerError(res: NextApiResponse, error: unknown, code = 500) {
  console.error(error)
