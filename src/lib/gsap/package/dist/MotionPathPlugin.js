;(function (global, factory) {
  ...
  function getRawPath(value) {
    value =
      _isString(value) && _selectorExp.test(value) ? document.querySelector(value) || value : value
  ...
  function getPositionOnPath(rawPath, progress, includeAngle, point) {
    ...
    length = segment.totalLength * progress
    i = segment.lookup.length
      ? segment.lookup[progress < 1 ? ~~(length / segment.minLength) : segment.lookup.length - 1] ||
        0
      : _getSampleIndex(samples, length, progress)
    min = i ? samples[i - 1] : 0
    max = samples[i]
    ...
    render: function render(ratio, data) {
      ...
      data.rotate &&
        data.rSet(
          data.target,
          data.rProp,
          rawPaths[0].angle * (data.radians ? _DEG2RAD$1 : 1) + data.rOffset + data.ru,
          data,
          ratio
        )
    },
