...
!(function (D, u) {
  ...
  function emojiSafeSplit(D, u, F) {
    ...
    for (var C, E, e = [], t = D.length, i = 0; i < t; i++)
      ((55296 <= (E = D.charAt(i)).charCodeAt(0) && E.charCodeAt(0) <= 56319) ||
        ...
        ((C = ((D.substr(i, 12).split(r) || [])[1] || '').length || 2),
        (E = D.substr(i, C)),
        (i += C - (e.emoji = 1))),
        e.push('>' === E ? '&gt;' : '<' === E ? '&lt;' : E)
    return e
  ...
  var u,
    ...
    E = C.toLowerCase(),
    B = {upperCase: new s(C), lowerCase: new s(E), upperAndLowerCase: new s(C + E)},
    e = {
      version: '3.7.1',
      name: 'scrambleText',
        ...
        if (
          ...
          return (
            ...
            (r.original = emojiSafeSplit(
              getText(D).replace(o, ' ').split('&nbsp;').join(''),
              C,
              n
            )),
            ('{original}' !== i && !0 !== i && null != i) || (i = r.original.join(C)),
            (r.text = emojiSafeSplit((i || '').replace(o, ' '), C, n)),
            ...
            (r.charSet = e = B[u.chars || 'upperCase'] || new s(u.chars)),
      ...
      render: function render(D, u) {
        ...
        g &&
          ...
          (e = D
            ...
            : M.join(h)),
          (o = p._from ? D : 1 - D),
          (a = x + (v ? (p._from ? o * o * o : 1 - o * o * o) : 1) * w),
          (e = _
            ? 1 !== D || (!p._from && 'isFromStart' !== p.data)
              ? ((n = A.slice(F).join(h)),
                (E = j
                  ? emojiSafeSplit(e)
                      .slice(0, (a - (S ? emojiSafeSplit(n) : n).length + 0.5) | 0)
                      .join('')
                  : e.substr(0, (a - (S ? emojiSafeSplit(n) : n).length + 0.5) | 0)),
                n)
              : ((E = ''), M.join(h))
          ...
          (n = H
            ? ((t = (r = _ ? b : T) && 0 != F) ? "<span class='" + r + "'>" : '') +
              E +
              (t ? '</span>' : '') +
              ((i = (s = _ ? T : b) && F !== I) ? "<span class='" + s + "'>" : '') +
              h +
              e +
              (i ? '</span>' : '')
            : E + h + e),
