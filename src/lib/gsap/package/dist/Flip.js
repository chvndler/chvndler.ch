;(function (global, factory) {
  ...
  var _id = 1,
    ...
    _makeAbsolute = function _makeAbsolute(elState, fallbackNode) {
      ...
      if (getProp('position') !== 'absolute') {
        ...
        if (!elState.isVisible || displayIsNone) {
          displayIsNone &&
            (_recordInlineStyles(elState, ['display']).display = fallbackNode.display)
    ...
    _handleCallback = function _handleCallback(callback, elStates, tl) {
      return (
        ...
        elStates.length &&
        tl.add(
          callback(_elementsFromElementStates(elStates), tl, new FlipState(elStates, 0, true)),
          0
        )
      )
    },
    _fit = function _fit(fromState, toState, scale, applyProps, fitChild, vars) {
      ...
      if (simple) {
        ...
        matrix = parentMatrix
          .clone()
          .multiply(toState.ctm ? toState.matrix.clone().multiply(toState.ctm) : toState.matrix)
        rotation = _round(Math.atan2(matrix.b, matrix.a) * _RAD2DEG)
        skewX = _round(Math.atan2(matrix.c, matrix.d) * _RAD2DEG + rotation) % 360
        scaleX = Math.sqrt(Math.pow(matrix.a, 2) + Math.pow(matrix.b, 2))
        scaleY =
          Math.sqrt(Math.pow(matrix.c, 2) + Math.pow(matrix.d, 2)) * Math.cos(skewX * _DEG2RAD)
  ...
  var ElementState = (function () {
    ...
    _proto2.update = function update(props, simple) {
      ...
      this.simple =
        simple || (_round(m.a) === 1 && !_round(m.b) && !_round(m.c) && _round(m.d) === 1)
      this.uncache = 0
    }
