;(function (global, factory) {
  ...
  var _numbersExp = /(?:(-)?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/gi,
    ...
    _createElement = function _createElement(type) {
      return _doc$1.createElementNS
        ? _doc$1.createElementNS('http://www.w3.org/1999/xhtml', type)
    ...
    _getConsolidatedMatrix = function _getConsolidatedMatrix(target) {
      return ((target.transform && target.transform.baseVal.consolidate()) || _identityMatrixObject)
        .matrix
    },
    ...
    _initCore = function _initCore() {
      ...
      _touchEventLookup = (function (types) {
        var standard = types.split(','),
          converted = (
            _tempDiv.onpointerdown !== undefined
              ? 'pointerdown,pointermove,pointerup,pointercancel'
              : _tempDiv.onmspointerdown !== undefined
    ...
    _onPress = function _onPress(e) {
      ...
      if (this._ctm.a === 1 && this._ctm.b === 0 && this._ctm.c === 0 && this._ctm.d === 1) {
        ...
        self._startPointerY =
          self._startPointerX * this._ctm.b + self._startPointerY * this._ctm.d + this._ctm.f
    ...
    _onRelease = function _onRelease(e, force) {
      ...
      if (
        ...
        (e &&
          self.touchID != null &&
          !force &&
          ((e.pointerId && e.pointerId !== self.touchID) ||
            (e.changedTouches && !_hasTouchID(e.changedTouches, self.touchID))))
      ) {
        return
      }
  ...
  var Anchor = (function () {
    ...
    _proto2.onDrag = function onDrag() {
      ...
      this.vars.onDrag.call(
        ...
        this,
        this._draggable.x - s[this.i],
        this._draggable.y - s[this.i + 1]
      )
    }
    ...
    _proto2.update = function update(rawPath, j, i) {
      ...
      this.smooth =
        ...
        i < segment.length - 2 &&
        Math.abs(
          Math.atan2(segment[i + 1] - segment[pi + 1], segment[i] - segment[pi]) -
            Math.atan2(segment[i + 3] - segment[i + 1], segment[i + 2] - segment[i])
        ) < 0.09
          ? 2
          : 0
  ...
  var PathEditor = (function () {
    ...
    _proto3.init = function init() {
      var pathData = this.path.getAttribute('d'),
        ...
        createAnchors =
          !this._rawPath ||
          rawPath.totalPoints !== this._rawPath.totalPoints ||
          rawPath.length !== this._rawPath.length,
        anchorVars = {
          callbackScope: this,
      ...
      if (createAnchors) {
        ...
        if (l > 1) {
          for (i = 1; i < l; i++) {
            this._anchors = this._anchors.concat(
              _createSegmentAnchors(rawPath, i, this, anchorVars)
            )
          }
        }
      } else {
      ...
      this._g.setAttribute(
        'transform',
        _getConcatenatedTransforms(this.path.parentNode) || 'translate(0,0)'
      )
      ...
      if (_ALT && _recentlyAddedAnchor !== anchor && this._editingAnchor) {
        ...
        if (anchor.smooth && (!isEnd || anchor.isClosedStart)) {
          ...
          if (length1 < 0.2) {
            ...
            angle1 =
              angle2 ||
              Math.atan2(segment[i + 7] - segment[pi - 3], segment[i + 6] - segment[pi - 4])
          }
    ...
    _proto3._updateAnchors = function _updateAnchors() {
      ...
      for (i = 0; i < this._anchors.length; i++) {
        this._anchors[i].element.style.fill =
      ...
      if (anchor && (i || anchor.isClosedStart)) {
        ...
        this._handle1.style.visibility = this._line1.style.visibility =
          !_ALT && x === segment[i] && y === segment[i + 1] ? 'hidden' : 'visible'

        this._handle1.setAttribute('transform', 'translate(' + x + _comma + y + ')')

        this._line1.setAttribute(
          'points',
          x + _comma + y + _comma + segment[i] + _comma + segment[i + 1]
        )
      } else {
      ...
      if (anchor && i < segment.length - 2) {
        ...
        this._line2.setAttribute(
          'points',
          segment[i] + _comma + segment[i + 1] + _comma + x + _comma + y
        )
      } else {
    ...
    _proto3._onPressHandle1 = function _onPressHandle1() {
      if (this._editingAnchor.smooth) {
        this._oppositeHandleLength = _getLength(
  ...
  PathEditor.simplifySVG = function (data, vars) {
    ...
    points =
      vars.curved === false && !/[achqstvz]/gi.test(data)
        ? data.match(_numbersExp)
        : stringToRawPath(data)[0]
  ...
  var gsap,
    ...
    _getPositionOnPage = function _getPositionOnPage(target) {
      var bounds = target.getBoundingClientRect(),
        windowOffsetY =
          _docEl.clientTop - (_win$2.pageYOffset || _docEl.scrollTop || _body$2.scrollTop || 0),
        windowOffsetX =
          _docEl.clientLeft - (_win$2.pageXOffset || _docEl.scrollLeft || _body$2.scrollLeft || 0)
      return {
    ...
    function MotionPathHelper(targetOrTween, vars) {
      ...
      if (!svg) {
        ...
        if (isSVG) {
          ...
          svg.style.cssText =
            'overflow:visible; background-color: transparent; position:absolute; width:100px; height:100px; top:' +
            (position.top - startY) +
            'px; left:' +
  ...
  MotionPathHelper.version = '3.7.1'
