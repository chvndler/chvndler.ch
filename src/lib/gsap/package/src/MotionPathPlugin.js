...
let _xProps = 'x,translateX,left,marginLeft,xPercent'.split(','),
  ...
  _getPropNum = (target, prop, unit) =>
    parseFloat(target._gsap.get(target, prop, unit || 'px')) || 0,
  _relativize = (segment) => {
    let x = segment[0],
      y = segment[1],
      i
  ...
  _originToPoint = (element, origin, parentMatrix) => {
    ...
    if ((element.tagName + '').toLowerCase() === 'svg') {
      ...
      y = svg.y
      svg.width ||
        (svg = {width: +element.getAttribute('width'), height: +element.getAttribute('height')})
    } else {
  ...
  _getAlignMatrix = (fromElement, toElement, fromOrigin, toOrigin) => {
    ...
    if (
      ...
      (toElement.getBBox &&
        fromElement.getBBox &&
        toElement.ownerSVGElement === fromElement.ownerSVGElement)
    ) {
  ...
  _sliceModifier = (start, end) => (rawPath) =>
    start || end !== 1 ? sliceRawPath(rawPath, start, end) : rawPath
...
export const MotionPathPlugin = {
    ...
    if (Array.isArray(path) && !('closed' in path) && typeof firstObj !== 'number') {
      ...
      if (x && y) {
        ...
        rawPaths.push(
          _segmentToRawPath(
            this,
            _populateSegmentFromArray(_populateSegmentFromArray([], path, x, 0), path, y, 1),
            target,
            x,
            y,
            slicer,
  ...
  render(ratio, data) {
    ...
    data.rotate &&
      data.rSet(
        data.target,
        data.rProp,
        rawPaths[0].angle * (data.radians ? _DEG2RAD : 1) + data.rOffset + data.ru,
        data,
        ratio
      )
  },
  ...
  getPositionOnPath,
  cacheRawPathMeasurements,
  convertToPath: (targets, swap) =>
    _toArray(targets).map((target) => convertToPath(target, swap !== false)),
  ...
  arrayToRawPath(value, vars) {
    vars = vars || {}
    let segment = _populateSegmentFromArray(
