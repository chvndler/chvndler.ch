...
let _win,
  ...
  _renderCSSProp = (ratio, data) =>
    data.set(data.t, data.p, Math.round((data.s + data.c * ratio) * 10000) / 10000 + data.u, data),
  _renderPropWithEnd = (ratio, data) =>
    data.set(
      data.t,
      data.p,
  ...
  _renderCSSPropWithBeginning = (ratio, data) =>
    data.set(
      data.t,
      data.p,
  ...
  _renderNonTweeningValue = (ratio, data) =>
    data.set(data.t, data.p, ratio ? data.e : data.b, data),
  _renderNonTweeningValueOnlyAtEnd = (ratio, data) =>
    data.set(data.t, data.p, ratio !== 1 ? data.b : data.e, data),
  ...
  _applySVGOrigin = (
    ...
    if (!originIsAbsolute) {
      ...
      x =
        xOrigin * (d / determinant) + yOrigin * (-c / determinant) + (c * ty - d * tx) / determinant
      y =
        xOrigin * (-b / determinant) + yOrigin * (a / determinant) - (a * ty - b * tx) / determinant
      xOrigin = x
      yOrigin = y
    }
  ...
  _parseTransform = (target, uncache) => {
    ...
    if (cache.svg) {
      t1 =
        ...
        !uncache &&
        target.getAttribute('data-svg-origin') // if origin is 0,0 and cache.uncache is true, let the recorded data-svg-origin stay. Otherwise, whenever we set cache.uncache to true, we'd need to set element.style.transformOrigin = (cache.xOrigin - bbox.x) + "px " + (cache.yOrigin - bbox.y) + "px". Remember, to work around browser inconsistencies we always force SVG elements' transformOrigin to 0,0 and offset the translation accordingly.
      _applySVGOrigin(
        target,
        t1 || origin,
    ...
    cache.x =
      x -
      ((cache.xPercent =
        x && (cache.xPercent || (Math.round(target.offsetWidth / 2) === Math.round(-x) ? -50 : 0)))
    ...
    cache.y =
      y -
      ((cache.yPercent =
        y && (cache.yPercent || (Math.round(target.offsetHeight / 2) === Math.round(-y) ? -50 : 0)))
  ...
  _addPxTranslate = (target, start, value) => {
    ...
    return (
      _round(parseFloat(start) + parseFloat(_convertToUnit(target, 'x', value + 'px', unit))) + unit
    )
  },
...
_forEachName('padding,margin,Width,Radius', (name, index) => {
  let t = 'Top',
    ...
    b = 'Bottom',
    l = 'Left',
    props = (index < 3 ? [t, r, b, l] : [t + l, t + r, b + r, b + l]).map((side) =>
      index < 2 ? name + side : 'border' + side + name
    )
