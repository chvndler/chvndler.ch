...
let gsap,
  ...
  _getDocScrollTop = (doc) =>
    _win.pageYOffset || doc.scrollTop || doc.documentElement.scrollTop || doc.body.scrollTop || 0,
  _getDocScrollLeft = (doc) =>
    _win.pageXOffset ||
    doc.scrollLeft ||
  ...
  _getMaxScroll = (element, axis) => {
    ...
    return Math.max(
      0,
      _isRoot(element)
        ? Math.max(_docElement[scroll], _body[scroll]) -
            (_win['inner' + dim] || _docElement[client] || _body[client])
  ...
  _setStyle = (element, property, value) => {
    ...
    if (value == null) {
      style.removeProperty &&
        style.removeProperty(property.replace(/([A-Z])/g, '-$1').toLowerCase())
    } else {
  ...
  _parseRect = (e) => {
    ...
    if (e === _win) {
      ...
      _tempRect.width = _tempRect.right =
        _docElement.clientWidth || e.innerWidth || _body.clientWidth || 0
      _tempRect.height = _tempRect.bottom =
        (e.innerHeight || 0) - 20 < _docElement.clientHeight
          ? _docElement.clientHeight
          : e.innerHeight || _body.clientHeight || 0
      return _tempRect
    }
  ...
  _getElementBounds = (element, context) => {
    ...
    if (element === _win) {
      ...
      right =
        left + (doc.documentElement.clientWidth || element.innerWidth || doc.body.clientWidth || 0)
      bottom =
        top +
        ((element.innerHeight || 0) - 20 < doc.documentElement.clientHeight
          ? doc.documentElement.clientHeight
          : element.innerHeight || doc.body.clientHeight || 0) //some browsers (like Firefox) ignore absolutely positioned elements, and collapse the height of the documentElement, so it could be 8px, for example, if you have just an absolutely positioned div. In that case, we use the innerHeight to resolve this.
      ...
      if (isSVG) {
        ...
        if (!width) {
          ...
          width =
            ...
            (bbox ? 0 : parseFloat(cs.borderLeftWidth) + parseFloat(cs.borderRightWidth))
          height =
            (parseFloat(cs.height) || element.clientHeight || 0) +
            (bbox ? 0 : parseFloat(cs.borderTopWidth) + parseFloat(cs.borderBottomWidth))
        }
      }
      right = width
      bottom = height
  ...
  _isClickable = (element) => {
    ...
    let data
    return !element || !element.getAttribute || element === _body
      ? false
  ...
  ScrollProxy = function (element, vars) {
    ...
    this.calibrate = function (force) {
      ...
      if (!widthMatches || force) {
        ...
        if (extraPadRight) {
          extraPadRight +=
            parseFloat(cs.paddingLeft) + (_addPaddingBR ? parseFloat(cs.paddingRight) : 0)
        }
      }
  ...
  _initCore = (required) => {
    if (_windowExists() && document.body) {
      ...
      _isTouchDevice =
        ('ontouchstart' in _docElement && 'orientation' in _win) ||
      ...
      _addPaddingBR = (function () {
        ...
        div.style.cssText = child.innerHTML =
...
class EventDispatcher {
  ...
  dispatchEvent(type) {
    ...
    ;(this._listeners[type] || []).forEach(
      (callback) =>
        callback.call(this, {type: type, target: this.target}) === false && (result = false)
    )
  ...
  constructor(target, vars) {
    ...
    let type = (vars.type || 'x,y').toLowerCase(),
            ...
            if (allowY) {
              ...
              if (gap < 0) {
                ...
                changeY = Math.min(
                  gap,
                  (autoScrollFactor *
                    (1 - Math.max(0, rect.bottom - pointerY) / autoScrollMarginBottom)) |
                    0
                )
                ...
                changeY = -Math.min(
                  parent.scrollTop,
                  (autoScrollFactor *
                    (1 - Math.max(0, pointerY - rect.top) / autoScrollMarginTop)) |
                    0
                )
              }
              if (changeY) {
            ...
            if (allowX) {
              ...
              if (gap < 0) {
                ...
                changeX = Math.min(
                  gap,
                  (autoScrollFactor *
                    (1 - Math.max(0, rect.right - pointerX) / autoScrollMarginRight)) |
                    0
                )
                ...
                changeX = -Math.min(
                  parent.scrollLeft,
                  (autoScrollFactor *
                    (1 - Math.max(0, pointerX - rect.left) / autoScrollMarginLeft)) |
                    0
                )
              }
              if (changeX) {
        ...
        if (scrollProxy) {
          ...
          if (rotationMode) {
            ...
            self.minX = minX = Math.round(
              getPropAsNum(xProp, 'px') + bounds.left - targetBounds.left - 0.5
            )
      ...
      animate = (inertia, forceZeroVelocity) => {
        ...
        if (inertia && InertiaPlugin) {
          if (inertia === true) {
            ...
            inertia = {
              resistance:
                (vars.throwResistance || vars.resistance || 1000) / (rotationMode ? 10 : 1),
            }
            if (rotationMode) {
          ...
          if (!inertia.duration) {
            inertia.duration = {
              ...
              min: !isNaN(vars.minDuration)
                ? vars.minDuration
                : overshootTolerance === 0 || (_isObject(inertia) && inertia.resistance > 1000)
                ? 0
                : 0.5,
        ...
        if (scrollProxy) {
          ...
          if (rotationMode) {
            parsedOrigin = target.ownerSVGElement
              ? [gsCache.xOrigin - target.getBBox().x, gsCache.yOrigin - target.getBBox().y]
      ...
      onPress = (e, force) => {
        ...
        allowNativeTouchScrolling =
          ...
          (self.vars.allowContextMenu && e && (e.ctrlKey || e.which > 2))
            ...
            : allowX
            ? 'y'
            : 'x' //note: in Chrome, right-clicking (for a context menu) fires onPress and it doesn't have the event.which set properly, so we must look for event.ctrlKey. If the user wants to allow context menus we should of course sense it here and not allow native touch scrolling.
          ...
          if ((self.lockAxis || self.lockedAxis) && (xChange || yChange)) {
            ...
            if (!temp) {
              self.lockedAxis = temp =
                allowX && Math.abs(xChange) > Math.abs(yChange) ? 'y' : allowY ? 'x' : null
      ...
      onRelease = (e, force) => {
        if (
          ...
          (e &&
            touchID != null &&
            !force &&
            ((e.pointerId && e.pointerId !== touchID && e.target !== target) ||
        ...
        if (isContextMenuRelease && originalEvent) {
          ...
          if (
            !self.allowEventDefault &&
            originalEvent &&
            (vars.dragClickables !== false || !isClickable.call(self, originalEvent.target)) &&
            wasDragging &&
            (!allowNativeTouchScrolling ||
              (touchDragAxis && allowNativeTouchScrolling === touchDragAxis)) &&
            originalEvent.cancelable !== false
      ...
      onClick = (e) => {
        ...
        if (
          (alreadyDispatched || (recentlyDragged && self.vars.suppressClickOnDrag !== false)) &&
    ...
    if (~type.indexOf('scroll')) {
      scrollProxy = this.scrollProxy = new ScrollProxy(
        target,
        _extend(
          {
            onKill: function () {
