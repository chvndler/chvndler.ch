...
let gsap,
  ...
  _parseEnd = (curProp, end, max, min, name, radius, velocity) => {
    ...
    if (_isObject(end)) {
      ...
      adjustedEnd = end.calculated
        ...
        : (_isFunction(endVar)
            ? endVar(end, velocity)
            : _getClosest(end, endVar, max, min, radius)) || end
      if (!end.calculated) {
      ...
      adjustedEnd = _isFunction(endVar)
        ? endVar(end, velocity)
        : _isArray(endVar)
        ? _getClosest(end, endVar, max, min, radius)
        : parseFloat(endVar)
    }
    if (adjustedEnd > max) {
      adjustedEnd = max
  ...
  _getNumOrDefault = (vars, property, defaultValue) =>
    isNaN(vars[property]) ? defaultValue : +vars[property],
  _calculateChange = (velocity, duration) => (duration * _checkPoint * velocity) / _checkPointRatio,
  _calculateDuration = (start, end, velocity) =>
    Math.abs(((end - start) * _checkPointRatio) / velocity / _checkPoint),
  _reservedProps = {
    resistance: 1,
    checkpoint: 1,
    preventOvershoot: 1,
    ...
    if (vars.linkedProps) {
      ...
      for (i = 0; i < linkedPropNames.length; i++) {
        ...
        if (curProp) {
          ...
          linkedProps[p] =
            parseFloat(getVal(target, p)) + _calculateChange(curVelocity, curDuration)
        }
      }
      return linkedProps
    }
      ...
      if (!_reservedProps[p]) {
        ...
        if (_isObject(curProp)) {
          ...
          curDuration = _clamp(
            minDuration,
            maxDuration,
            Math.abs(curVelocity / _getNumOrDefault(curProp, 'resistance', resistance))
          )
  ...
  _initCore = () => {
    ...
    if (gsap) {
      ...
      gsap.config({
        resistance: 100,
        unitFactors: {time: 1000, totalTime: 1000, progress: 1000, totalProgress: 1000},
      })
      _config = gsap.config()
    ...
    let cache = target._gsap,
      ...
      duration = _isNumber(dur)
        ...
        : _calculateTweenDuration(
            ...
            durIsObj && 'overshoot' in dur ? +dur.overshoot : preventOvershoot ? 0 : 1,
            true
          ),
      p,
      curProp,
...
'track,untrack,isTracking,getVelocity,getByTarget'
  .split(',')
  .forEach((name) => (InertiaPlugin[name] = VelocityTracker[name]))
