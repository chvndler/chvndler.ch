...
let _id = 1,
  ...
  _toggleClass = (targets, className, action) =>
    targets.forEach((el) => el.classList[action](className)),
  _reserved = {
    zIndex: 1,
    clear: 1,
    simple: 1,
  ...
  _findElStateInState = (state, other) =>
    (other && state.idLookup[_parseElementState(other).id]) || state.elementStates[0],
  _parseElementState = (elOrNode, props, simple, other) =>
    elOrNode instanceof ElementState
      ? elOrNode
  ...
  _getCTMInverse = (el) =>
    el.getCTM && el.nodeName.toLowerCase() === 'svg' && _getCTM(el).inverse(),
  _elementsFromElementStates = (elStates) => elStates.map((elState) => elState.element),
  _handleCallback = (callback, elStates, tl) =>
    callback &&
    elStates.length &&
    tl.add(callback(_elementsFromElementStates(elStates), tl, new FlipState(elStates, 0, true)), 0),
  ...
  _parseState = (targetsOrState, vars) =>
    targetsOrState instanceof FlipState ? targetsOrState : new FlipState(targetsOrState, vars),
  _getChangingElState = (toState, fromState, id) => {
    let to1 = toState.idLookup[id],
      to2 = toState.alt[id]
    return to2.isVisible &&
      (!(fromState.getElementState(to2.element) || to2).isVisible || !to1.isVisible)
      ? to2
      : to1
  },
  _fromTo = (fromState, toState, vars, relative) => {
    ...
    props &&
      (_memoizedProps[props] || _memoizeProps(props)).forEach(
        (p) => (tweenVars[p] = (i) => comps[i].a.props[p])
      )
    comps.finalStates = finalStates = []

    absolute &&
      _orderByDOMDepth(comps, true).forEach(
        (c) => (c.a.isVisible || c.b.isVisible) && _makeAbsolute(c.sd < 0 ? c.b : c.a, c.b)
    ...
    fade &&
      (tweenVars.opacity = (i) =>
        comps[i].sd < 0 ? 0 : comps[i].sd > 0 ? comps[i].a.opacity : '+=0')

    if (swapOutTargets.length) {
      stagger = gsap.utils.distribute(stagger)
      let dummyArray = targets.slice(swapOutTargets.length)
      tweenVars.stagger = (i, el) =>
        stagger(~swapOutTargets.indexOf(el) ? targets.indexOf(comps[i].swap.t) : i, el, dummyArray)
    }
    ...
    _callbacks.forEach(
      (name) => vars[name] && animation.eventCallback(name, vars[name], vars[name + 'Params'])
    ...
    if (targets.length || leaving.length || entering.length) {
      toggleClass &&
        animation.add(
          () => _toggleClass(targets, toggleClass, animation._zTime < 0 ? 'remove' : 'add'),
          0
        ) &&
        !paused &&
...
export class Flip {
  ...
  static makeAbsolute(targetsOrStates, vars) {
    return (
      targetsOrStates instanceof FlipState ? targetsOrStates : new FlipState(targetsOrStates, vars)
    ).makeAbsolute()
  }
