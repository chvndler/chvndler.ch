...
let gsap,
  ...
  _getProxyProp = (element, property) =>
    ~_proxies.indexOf(element) && _proxies[_proxies.indexOf(element) + 1][property],
  ...
  _getOffsetsFunc = (element, isViewport) =>
    !isViewport || ~_proxies.indexOf(element) ? _getBoundsFunc(element) : () => _winOffsets,
  ...
  _horizontal = {
    ...
    sc: function (value) {
      return arguments.length
        ...
        : _win.pageXOffset || _doc[_scrollLeft] || _docEl[_scrollLeft] || _body[_scrollLeft] || 0
    },
  },
  _vertical = {
    s: _scrollTop,
    ...
    sc: function (value) {
      return arguments.length
        ...
        : _win.pageYOffset || _doc[_scrollTop] || _docEl[_scrollTop] || _body[_scrollTop] || 0
    },
  },
  ...
  _createMarker = (
    ...
    ;(isScroller || !useFixedPosition) &&
      (css +=
        (direction === _vertical ? _right : _bottom) + ':' + (offset + parseFloat(indent)) + 'px;')
    matchWidthEl &&
      (css += 'box-sizing:border-box;text-align:left;width:' + matchWidthEl.offsetWidth + 'px;')
    e._isStart = isStart
  ...
  _onResize = () =>
    !_refreshing && !_ignoreResize && !_doc.fullscreenElement && _resizeDelay.restart(true), // ignore resizes triggered by refresh()
  _listeners = {},
  _emptyArray = [],
  _media = [],
    ...
    if (_lastMediaTick !== tick || _startup) {
      ...
      for (; i < _media.length; i += 4) {
        ...
        if (index !== _media[i + 3]) {
          ...
          index
            ? matches.push(i)
            : _revertAll(1, _media[i]) || (_isFunction(_media[i + 2]) && _media[i + 2]()) // Firefox doesn't update the "matches" property of the MediaQueryList object correctly - it only does so as it calls its change handler - so we must re-create a media query here to ensure it's accurate.
        }
      }
  ...
  _softRefresh = () =>
    _removeListener(ScrollTrigger, 'scrollEnd', _softRefresh) || _refreshAll(true),
...
export class ScrollTrigger {
  ...
  init(vars, animation) {
    ...
    vars = _setDefaults(
      _isString(vars) || _isNumber(vars) || vars.nodeType ? {trigger: vars} : vars,
      _defaults
    )
              ...
              tweenTo(
                endScroll,
                {
                  duration: snapDurClamp(
                    _abs(
                      (Math.max(_abs(naturalEnd - totalProgress), _abs(endValue - totalProgress)) *
                        0.185) /
                        velocity /
                        0.05 || 0
                    )
                  ...
                  onInterrupt: () =>
                    snapDelayedCall.restart(true) && onInterrupt && onInterrupt(self),
                  onComplete: () => {
    ...
    if (pin) {
      pinSpacing === false ||
        pinSpacing === _margin ||
        (pinSpacing =
          !pinSpacing && _getComputedStyle(pin.parentNode).display === 'flex' ? false : _padding) // if the parent is display: flex, don't apply pinSpacing by default.
      self.pin = pin
      ...
      if (r !== self.isReverted) {
        ...
        markerStart &&
          [markerStart, markerEnd, markerStartTrigger, markerEndTrigger].forEach(
            (m) => (m.style.display = r ? 'none' : 'block')
          )
        r && (_refreshing = 1)
      ...
      if (clipped !== prevProgress && self.enabled) {
        ...
        toggleClass &&
          ...
          _toArray(toggleClass.targets).forEach((el) =>
            el.classList[isActive || once ? 'add' : 'remove'](toggleClass.className)
          ) // classes could affect positioning, so do it even if reset or refreshing is true.
        onUpdate && !isToggle && !reset && onUpdate(self)
        if (stateChanged && !_refreshing) {
          toggleState =
            clipped && !prevProgress ? 0 : clipped === 1 ? 1 : prevProgress === 1 ? 2 : 3 // 0 = enter, 1 = leave, 2 = enterBack, 3 = leaveBack (we prioritize the FIRST encounter, thus if you scroll really fast past the onEnter and onLeave in one tick, it'd prioritize onEnter.
          if (isToggle) {
            action =
              (!toggled &&
