...
let gsap,
  ...
  _parsePath = (path, target, vars) =>
    _isString(path) && _selectorExp.test(path)
      ? _doc.querySelector(path)
      : Array.isArray(path)
      ? _rawPathToString(
          _arrayToRawPath(
            [{x: gsap.getProperty(target, 'x'), y: gsap.getProperty(target, 'y')}, ...path],
            vars
          )
        )
  ...
  _getConsolidatedMatrix = (target) =>
    (target.transform.baseVal.consolidate() || _identityMatrixObject).matrix,
