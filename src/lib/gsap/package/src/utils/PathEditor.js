...
let _numbersExp = /(?:(-)?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/gi,
  _doc,
  _supportsPointer,
  _win,
  _body,
  ...
  _getConsolidatedMatrix = (target) =>
    ((target.transform && target.transform.baseVal.consolidate()) || _identityMatrixObject).matrix,
...
export class PathEditor {
  constructor(target, vars) {
    ...
    this._selection =
      vars._selection || _createSVG('g', this._g, {class: 'path-editor-selection path-editor'})
