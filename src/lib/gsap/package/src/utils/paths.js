...
let _svgPathExp = /[achlmqstvz]|(-?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/gi,
  ...
  _temp2 = {},
  _roundingNum = 1e5,
  _wrapProgress = (progress) =>
    Math.round(((progress + _largeNum) % 1) * _roundingNum) / _roundingNum ||
    (progress < 0 ? 0 : 1), //if progress lands on 1, the % will make it 0 which is why we || 1, but not if it's negative because it makes more sense for motion to end at 0 in that case.
...
export function getPositionOnPath(rawPath, progress, includeAngle, point) {
  ...
  length = segment.totalLength * progress
  i = segment.lookup.length
    ? segment.lookup[progress < 1 ? ~~(length / segment.minLength) : segment.lookup.length - 1] || 0
    : _getSampleIndex(samples, length, progress)
  min = i ? samples[i - 1] : 0
  max = samples[i]
  if (max < length) {
