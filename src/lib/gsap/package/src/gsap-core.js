...
let _config = {
  ...
  _addGlobal = (name, obj) =>
    (name && (_globals[name] = obj) && _installScope && (_installScope[name] = obj)) || _globals,
  _emptyFunc = () => 0,
  _reservedProps = {},
  _lazyTweens = [],
  _lazyLookup = {},
  ...
  _getProperty = (target, property, v) =>
    (v = target[property]) && _isFunction(v)
      ? target[property]()
  ...
  _hasNoPausedAncestors = (animation) =>
    !animation || (animation._ts && _hasNoPausedAncestors(animation.parent)),
  _elapsedCycleDuration = (animation) =>
    animation._repeat
      ? _animationCycle(animation._tTime, (animation = animation.duration() + animation._rDelay)) *
        animation
      : 0,
  ...
  _scrollTrigger = (animation, trigger) =>
  ...
  _parentPlayheadIsBeforeStart = ({parent}) =>
    parent &&
    parent._ts &&
    parent._initted &&
    !parent._lock &&
  ...
  _onUpdateTotalDuration = (animation) =>
    animation instanceof Timeline ? _uncache(animation) : _setDuration(animation, animation._dur),
  ...
  _isArrayLike = (value, nonEmpty) =>
    ...
    !value.nodeType &&
    value !== _win,
  _flatten = (ar, leaveStrings, accumulator = []) =>
    ar.forEach((value) =>
      (_isString(value) && !leaveStrings) || _isArrayLike(value, 1)
        ? accumulator.push(...toArray(value))
        : accumulator.push(value)
    ) || accumulator,
  //takes any value and returns an array. If it's a string (and leaveStrings isn't true), it'll use document.querySelectorAll() and convert that to an array. It'll also accept iterables like jQuery objects.
  toArray = (value, scope, leaveStrings) =>
  ...
  shuffle = (a) => a.sort(() => 0.5 - Math.random()), // alternative that's a bit faster and more reliably diverse but bigger:   for (let j, v, i = a.length; i; j = Math.floor(Math.random() * i), v = a[--i], a[i] = a[j], a[j] = v); return a;
  ...
  random = (min, max, roundingIncrement, returnFunction) =>
    _conditionalReturn(
      _isArray(min)
        ? !max
      ...
      () =>
        _isArray(min)
          ...
          : (roundingIncrement = roundingIncrement || 1e-5) &&
            (returnFunction =
              roundingIncrement < 1 ? 10 ** ((roundingIncrement + '').length - 2) : 1) &&
            Math.floor(
              Math.round(
                (min -
                  roundingIncrement / 2 +
  ...
  pipe =
    (...functions) =>
    (value) =>
      functions.reduce((v, f) => f(v), value),
  ...
  wrap = function (min, max, value) {
    ...
    return _isArray(min)
      ...
      : _conditionalReturn(value, (value) => ((range + ((value - min) % range)) % range) + min)
  },
  wrapYoyo = (min, max, value) => {
    ...
    return _isArray(min)
      ...
      : _conditionalReturn(value, (value) => {
  ...
  _insertEase = (
    ...
    easeInOut = (p) => (p < 0.5 ? easeIn(p * 2) / 2 : 1 - easeIn((1 - p) * 2) / 2)
  ) => {
    ...
    _forEachName(names, (name) => {
      ...
      for (let p in ease) {
        _easeMap[lowercaseName + (p === 'easeIn' ? '.in' : p === 'easeOut' ? '.out' : '.inOut')] =
          _easeMap[name + '.' + p] = ease[p]
      }
    })
    return ease
  },
  _easeInOutFromOut = (easeOut) => (p) =>
...
_forEachName(
  ...
  (name) => (_callbackNames += name + ',' + name + 'Params,')
)
...
export class Animation {
  ...
  timeScale(value) {
    ...
    let tTime =
      this.parent && this._ts ? _parentToChildTotalTime(this.parent._time, this) : this._tTime // make sure to do the parentToChildTotalTime() BEFORE setting the new _ts because the old one must be used in that calculation.
  ...
  endTime(includeRepeats) {
    return (
      this._start +
      (_isNotFalse(includeRepeats) ? this.totalDuration() : this.duration()) / Math.abs(this._ts)
    )
  }
...
export class Timeline extends Animation {
  ...
  render(totalTime, suppressEvents, force) {
    let prevTime = this._time,
      ...
      dur = this._dur,
      tTime =
        this !== _globalTimeline && totalTime > tDur - _tinyNum && totalTime >= 0
          ? tDur
          : totalTime < _tinyNum
          ? 0
          : totalTime,
  ...
  totalTime(totalTime, suppressEvents) {
    ...
    if (!this._dp && this._ts) {
      ...
      this._start = _round(
        _ticker.time -
          (this._ts > 0 ? totalTime / this._ts : (this.totalDuration() - totalTime) / -this._ts)
      )
    }
  ...
  tweenFromTo(fromPosition, toPosition, vars) {
    return this.tweenTo(
      toPosition,
      _setDefaults({startAt: {time: _parsePosition(this, fromPosition)}}, vars)
    )
  }
...
let _addComplexStringPropTween = function (
    ...
    while ((result = _complexStringNumExp.exec(end))) {
      ...
      if (endNum !== startNums[matchIndex++]) {
        ...
        pt._pt = {
          ...
          c:
            endNum.charAt(1) === '='
              ? parseFloat(endNum.substr(2)) * (endNum.charAt(0) === '-' ? -1 : 1)
  ...
  _initTween = (tween, time) => {
    ...
    tween._yEase = yoyoEase
      ? _invertEase(_parseEase(yoyoEase === true ? ease : yoyoEase, _defaults.ease))
      : 0
  ...
  _parseFuncOrString = (value, tween, i, target, targets) =>
    _isFunction(value)
...
export class Tween extends Animation {
  constructor(targets, vars, position, skipInherit) {
    ...
    let {
      ...
      parsedTargets = (
        _isArray(targets) || _isTypedArray(targets) ? _isNumber(targets[0]) : 'length' in vars
      )
        ? [targets]
        : toArray(targets), // edge case: someone might try animating the "length" of an object with a "length" property that's initially set to 0 so don't interpret that as an empty Array-like object.
      tl,
      i,
      copy,
      l,
    ...
    if (keyframes || stagger || _isFuncOrString(duration) || _isFuncOrString(delay)) {
      ...
      if (keyframes) {
        ...
        stagger
          ? parsedTargets.forEach((t, i) =>
              keyframes.forEach((frame, j) => tl.to(t, frame, j ? '>' : i * stagger))
            )
        ...
        for (i = 0; i < l; i++) {
          ...
          copy.delay =
            (+_parseFuncOrString(delay, this, i, curTarget, parsedTargets) || 0) - this._delay
    ...
    if (
      ...
      (!duration &&
        !keyframes &&
        this._start === _round(parent._time) &&
  ...
  render(totalTime, suppressEvents, force) {
    let prevTime = this._time,
      ...
      dur = this._dur,
      tTime =
        totalTime > tDur - _tinyNum && totalTime >= 0 ? tDur : totalTime < _tinyNum ? 0 : totalTime,
      time,
      pt,
      iteration,
      cycleDuration,
    ...
    if (!dur) {
      ...
      ;(timeline &&
        timeline.render(
          totalTime < 0 ? totalTime : !time && isYoyo ? -_tinyNum : timeline._dur * ratio,
          suppressEvents,
          force
        )) ||
...
let _setterPlain = (target, property, value) => (target[property] = value),
  ...
  _getSetter = (target, property) =>
    _isFunction(target[property])
      ? _setterFunc
      : _isUndefined(target[property]) && target.setAttribute
      ? _setterAttribute
      : _setterPlain,
  _renderPlain = (ratio, data) =>
    data.set(data.t, data.p, Math.round((data.s + data.c * ratio) * 1000000) / 1000000, data),
