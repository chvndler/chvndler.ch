...
let gsap,
  ...
  _clean = (value, index, target, targets) => {
    ...
    if (typeof value !== 'object') {
      return _isString(value) && value !== 'max' && value.charAt(1) !== '='
        ? {x: value, y: value}
        : {y: value} //if we don't receive an object as the parameter, assume the user intends "y".
      ...
      for (p in value) {
        result[p] =
          p !== 'onAutoKill' && _isFunction(value[p]) ? value[p](index, target, targets) : value[p]
      }
      return result
    }
  },
  _getOffset = (element, container) => {
    ...
    let rect = element.getBoundingClientRect(),
      ...
      cRect = isRoot
        ? {
            top:
              _docEl.clientTop - (_window.pageYOffset || _docEl.scrollTop || _body.scrollTop || 0),
            left:
              _docEl.clientLeft -
              (_window.pageXOffset || _docEl.scrollLeft || _body.scrollLeft || 0),
          }
  ...
  _parseVal = (value, target, axis, currentVal, offset) =>
    !isNaN(value) && typeof value !== 'object'
