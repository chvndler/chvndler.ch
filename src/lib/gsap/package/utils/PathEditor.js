  ...
  _proto2.update = function update(rawPath, j, i) {
    ...
    this.smooth =
      ...
      Math.abs(
        ...
        ? 2
        : 0 //0: corner, 1: smooth but not mirrored, 2: smooth and mirrored.
    ...
    if (_ALT && _recentlyAddedAnchor !== anchor && this._editingAnchor) {
      ...
      if (anchor.smooth && (!isEnd || anchor.isClosedStart)) {
        ...
        if (length1 < 0.2) {
          ...
          angle1 =
            angle2 || Math.atan2(segment[i + 7] - segment[pi - 3], segment[i + 6] - segment[pi - 4])
        }

        if (length2 < 0.2) {
          length2 = _getLength(segment, i, i + 6) / 4
          angle2 =
            angle1 || Math.atan2(segment[i + 7] - segment[pi - 3], segment[i + 6] - segment[pi - 4])
        }
  ...
  _proto3._updateAnchors = function _updateAnchors() {
    ...
    for (i = 0; i < this._anchors.length; i++) {
      this._anchors[i].element.style.fill =
        this._selectedAnchors.indexOf(this._anchors[i]) !== -1 ? _selectionColor : 'white' //this._anchors[i].element.setAttribute("fill", (this._selectedAnchors.indexOf(this._anchors[i]) !== -1) ? _selectionColor : "white");
    }
