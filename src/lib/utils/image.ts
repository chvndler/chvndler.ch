...
const imageWidths = [16, 32, 48, 64, 96, 128, 256, 384, 640, 750, 828, 1080, 1200, 1920, 2048, 3840] as const;
...
export const findClosestNextImageWidth = (width: number): NextImageWidth => {
  return (imageWidths.find((w) => w >= width) || imageWidths[imageWidths.length - 1]) ?? 3840;
};
