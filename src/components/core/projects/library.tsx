  ...
  const listItems = sortedProjects.map((project, i) => (
    <li
      ...
      <div
        ...
        <Link
          href={project.url}
          className='text-md items-center justify-start py-2 font-favorit font-[500] lowercase tracking-tight text-carbon-500 hover:text-carbon-700 dark:hover:text-carbon-300 md:text-lg'>
          {project.title}
        </Link>
        ...
        <div
          ...
          {project.tags.map((tag, ti) => (
            <TagChip
              key={ti}
              className='hidden border border-slate-400 bg-blackA3 font-favorit text-[8.5px] font-[600] uppercase text-blackA9 dark:border-slate-600 dark:bg-whiteA2 dark:text-whiteA8 sm:inline'>
              {tag}
            </TagChip>
          ))}
        </div>
...
function FormattedDate({ postDate }: { postDate: Projects }) {
  ...
  return (
    <p className='mr-2 text-right font-favorit text-[10px] font-semibold uppercase text-blackA10 dark:text-whiteA8'>
