  ...
  const listItems = sortedProjects.map((project, i) => (
    <li
      ...
      <div key={i} className='flex flex-row items-center justify-between w-full m-auto'>
        <Link
          href={project.url}
          className='text-md items-center justify-start py-2 font-unica font-[500] lowercase tracking-tight text-carbon-500 hover:text-carbon-700 dark:hover:text-carbon-300 md:text-lg'>
          {project.title}
        </Link>
...
function FormattedDate({ postDate }: { postDate: Projects }) {
  ...
  return (
    <p className='mr-2 text-right font-unica text-[10px] font-semibold uppercase text-blackA10 dark:text-whiteA8'>
